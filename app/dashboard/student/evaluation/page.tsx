...
export default function StudentEvaluationPage() {
    ...
    async function load() {
        ...
        setError("")
        try {
            // IMPORTANT: only student-safe endpoint
    ...
    const panelistRows = React.useMemo(() => {
        return selected?.panelistEvaluations ?? []
    }, [selected?.panelistEvaluations])

    const derivedSelectedScores = React.useMemo(() => {
        const base = selected?.scores ?? { groupScore: null, systemScore: null, personalScore: null }

        const groupAvg = averageNumbers(panelistRows.map((r) => toNumber(r.scores.groupScore)))
        const systemAvg = averageNumbers(panelistRows.map((r) => toNumber(r.scores.systemScore)))
        const personalAvg = averageNumbers(panelistRows.map((r) => toNumber(r.scores.personalScore)))

        return {
            groupScore: groupAvg !== null ? groupAvg : base.groupScore,
            systemScore: systemAvg !== null ? systemAvg : base.systemScore,
            personalScore: personalAvg !== null ? personalAvg : base.personalScore,
        }
    }, [selected?.scores, panelistRows])
                                ...
                                <div className="grid gap-4 md:grid-cols-3">
                                    <Card>
                                        <CardHeader className="space-y-1">
                                            ...
                                            <CardDescription>Panelists’ overall score.</CardDescription>
                                        </CardHeader>
                                    ...
                                    <Card>
                                        <CardHeader className="space-y-1">
                                            ...
                                            <CardDescription>Panelists’ system score.</CardDescription>
                                        </CardHeader>
                                        <CardContent className="space-y-2">
                                    ...
                                    <Card>
                                        <CardHeader className="space-y-1">
                                            ...
                                            <CardDescription>Your individual score (from evaluation_extras).</CardDescription>
                                        </CardHeader>
                                        <CardContent className="space-y-2">
                                            ...
                                            {toNumber(derivedSelectedScores.personalScore) === null ? (
                                                <div className="text-xs text-muted-foreground">
                                                    Not available yet (panelists may not have submitted your individual score).
                                                </div>
                                            ) : null}
                                        </CardContent>
                                    </Card>
                        ...
                        <TabsContent value="breakdown" className="space-y-4">
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>Shows Group/System/You per evaluator.</CardDescription>
                                </CardHeader>
                                <CardContent className="space-y-4">
                                    {panelistRows.length === 0 ? (
                                        <Alert>
