...
export default function StudentEvaluationDetailPage() {
    ...
    const rows = React.useMemo(() => {
        return item?.panelistEvaluations ?? []
    }, [item?.panelistEvaluations])

    const derivedScores = React.useMemo(() => {
        const base = item?.scores ?? { groupScore: null, systemScore: null, personalScore: null }

        const groupAvg = averageNumbers(rows.map((r) => toNumber(r.scores.groupScore)))
        const systemAvg = averageNumbers(rows.map((r) => toNumber(r.scores.systemScore)))
        const personalAvg = averageNumbers(rows.map((r) => toNumber(r.scores.personalScore)))
                    ...
                    <Card>
                        <CardHeader className="space-y-2">
                            <CardTitle className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                ...
                                <div className="flex items-center gap-2">{statusBadge(item.schedule.status)}</div>
                            </CardTitle>
                            <CardDescription className="space-y-1">
                                <div className="text-xs text-muted-foreground">
                                    When: {formatDateTime(item.schedule.scheduledAt)}{" "}
                                    {item.schedule.room ? <> · Room {item.schedule.room}</> : null}
                                </div>
                            ...
                            <div className="grid gap-4 md:grid-cols-3">
                                ...
                                <Card>
                                    ...
                                    <CardContent className="space-y-2">
                                        ...
                                        {toNumber(derivedScores.personalScore) === null ? (
                                            <div className="text-xs text-muted-foreground">
