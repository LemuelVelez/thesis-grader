...
export default function StaffRubricDetailPage() {
    ...
    const load = React.useCallback(async () => {
        ...
        try {
            ...
            // Staff API only guarantees list endpoint.
            // 1) Best-effort: try a single GET by query (?id=)
            try {
                const one = await apiFetch<any>(`/api/staff/rubric-templates?id=${encodeURIComponent(templateId)}`)
                ...
                // 2) fallback: list all then find
                const all = await apiFetch<any>("/api/staff/rubric-templates")
            ...
            try {
                const crit = await apiFetch<any>(`/api/staff/rubric-criteria?templateId=${encodeURIComponent(templateId)}`)
                setCriteria(normalizeList(crit))
            } catch {
                const all = await apiFetch<any>("/api/staff/rubric-criteria")
                const list = normalizeList(all)
    ...
    async function onSaveTemplateForm() {
        ...
        try {
            ...
            const updated = await apiFetch<any>(`/api/staff/rubric-templates/${encodeURIComponent(templateId)}`, {
                method: "PUT",
    ...
    async function onDeleteTemplate() {
        setError(null)
        try {
            await apiFetch(`/api/staff/rubric-templates/${encodeURIComponent(templateId)}`, { method: "DELETE" })
            router.push("/dashboard/staff/rubrics")
        } catch (e: any) {
    ...
    async function onSaveCriterion() {
        ...
        try {
            ...
            if (isEdit) {
                await apiFetch(`/api/staff/rubric-criteria/${encodeURIComponent(cid)}`, {
                    method: "PUT",
                    body: JSON.stringify(payload),
                })
            } else {
                await apiFetch(`/api/staff/rubric-criteria`, {
                    method: "POST",
                    body: JSON.stringify(payload),
                })
            }
    ...
    async function onDeleteCriterion(cid: string) {
        setError(null)
        try {
            await apiFetch(`/api/staff/rubric-criteria/${encodeURIComponent(cid)}`, { method: "DELETE" })
            await load()
        } catch (e: any) {
                ...
                <div className="flex flex-col gap-3">
                    <Breadcrumb>
                        <BreadcrumbList>
                            ...
                            <BreadcrumbItem>
                                <BreadcrumbLink asChild>
                                    <Link href="/dashboard/staff/rubrics">Rubrics</Link>
                                </BreadcrumbLink>
                            </BreadcrumbItem>
                            <BreadcrumbSeparator />
                            <BreadcrumbItem>
                            ...
                            <AlertDialog>
                                ...
                                <AlertDialogContent>
                                    <AlertDialogHeader>
                                        ...
                                        <AlertDialogDescription>
                                            This action cannot be undone. Any criteria attached to this rubric template may be deleted as well (via DB cascade).
                                        </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
                                    ...
                                    <DialogContent className="sm:max-w-4xl">
                                        ...
                                        <div className="grid gap-6 py-2 md:grid-cols-2">
                                            ...
                                            <div className="grid gap-2">
                                                ...
                                                />
                                                {criterionJsonError ? <p className="text-sm text-destructive">{criterionJsonError}</p> : null}
                                                <p className="text-xs text-muted-foreground">
                                                                ...
                                                                <TableCell className="text-right">
                                                                    <DropdownMenu>
                                                                        ...
                                                                        <DropdownMenuContent align="end">
                                                                            ...
                                                                            <DropdownMenuSeparator />
                                                                            <DropdownMenuItem onClick={() => openEditCriterion(c)}>Edit</DropdownMenuItem>
                                                                            <DropdownMenuSeparator />
                            ...
                            <CardContent className="pt-6">
                                {loading ? (
                                    ...
                                    <div className="grid gap-3">
                                        ...
                                        />
                                        {templateJsonError ? <p className="text-sm text-destructive">{templateJsonError}</p> : null}
